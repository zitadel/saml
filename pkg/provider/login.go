...
func (p *IdentityProvider) callbackHandleFunc(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logging.Error(err)
		response.sendBackResponse(r, w, response.MakeFailedResponse(StatusCodeRequestDenied, fmt.Errorf("failed to get request: %w", err).Error(), p.timeFormat))
		return
	}
	...
	case PostBinding:
		if err := createPostSignature(r.Context(), samlResponse, p); err != nil {
			logging.Error(err)
			response.sendBackResponse(r, w, response.MakeFailedResponse(StatusCodeResponder, fmt.Errorf("failed to sign response: %w", err).Error(), p.timeFormat))
			return
		}
